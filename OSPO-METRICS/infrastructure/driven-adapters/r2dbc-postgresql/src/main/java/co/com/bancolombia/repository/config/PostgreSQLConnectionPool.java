package co.com.bancolombia.repository.config;
...
public class PostgreSQLConnectionPool {
	...
	@Autowired
	@Qualifier("secretMetric")
	private SecretDBResponse secretMetric;

	@Bean
	public ConnectionPool getConnectionConfig() {
		SecretDBResponse secret = secretMetric;

		PostgresqlConnectionProperties pgProperties = new PostgresqlConnectionProperties();

		pgProperties.setDatabase(secret.getDbname());
		pgProperties.setHost(secret.getHost());
		pgProperties.setPort(Integer.parseInt(secret.getPort()));
		pgProperties.setUsername(secret.getUsername());
		pgProperties.setPassword(secret.getPassword());
		pgProperties.setSchema(secret.getDbInstanceIdentifier());
